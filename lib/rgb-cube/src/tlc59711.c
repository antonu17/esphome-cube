...
void IRAM_ATTR tlc_init(tlc_config_t *config) {
  ...
  _tlc_init_spi_driver(config->data_pin, config->sclk_pin, config->spi_mode,
...
void IRAM_ATTR tlc_set_led(int led, uint8_t r, uint8_t g, uint8_t b) {
  ...
  ESP_LOG_BUFFER_HEXDUMP(TAG, _spi_buffer, _num_drivers * TLC59711_BUFFER_SIZE,
                         ESP_LOG_VERBOSE);
}
...
void IRAM_ATTR tlc_wait_write() {
  ...
  // We could inspect rtrans now if we received any info back. The TLC59711 is
  // treated as write-only, though.
}

void IRAM_ATTR _tlc_init_spi_driver(int data_pin, int sclk_pin,
                                    uint8_t spi_mode, int spi_clock_speed_hz) {
...
void IRAM_ATTR _tlc_allocate_spi_buffer() {
  _spi_buffer =
      heap_caps_calloc(_num_drivers, TLC59711_BUFFER_SIZE, MALLOC_CAP_DMA);
...
uint8_t IRAM_ATTR _8bit_to_7bit(uint8_t val) { return val >> 1; }
