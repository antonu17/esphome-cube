...
void draw_positions_axis(fb_axis_t axis, uint8_t positions[64],
                         fb_shift_direction_t direction) {
  int x, y, p;
  ...
  for (x = 0; x < 8; x++) {
    for (y = 0; y < 8; y++) {
      ...
      if (axis == FB_AXIS_Z) fb_set_pixel(x, y, p, on);

      if (axis == FB_AXIS_Y) fb_set_pixel(x, p, y, on);

      if (axis == FB_AXIS_X) fb_set_pixel(p, y, x, on);
    }
  }
}
...
void _shift_suspend(fb_axis_t axis, fb_shift_direction_t direction) {
  ...
  for (i = 0; i < 8; i++) {
    // For every iteration, move all position one step closer to their
    // destination
    for (px = 0; px < 64; px++) {
